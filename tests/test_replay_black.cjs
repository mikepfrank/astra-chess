...
(async () => {
  ...
  try {
    ...
    const screenshotDir = path.join(__dirname, '../images/replays');
    await fs.mkdir(screenshotDir, {recursive:true});
    await seek(92);
    await page.screenshot({path:path.join(screenshotDir, 'wally-engine-v02-black-desktop.png'), fullPage:true, animations:'disabled'});
    ...
    await page.screenshot({path:path.join(screenshotDir, 'wally-engine-v02-black-mobile.png'), fullPage:true, animations:'disabled'});
