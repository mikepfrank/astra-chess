...
Two actions use the same persistent synthetic thread across a process restart.
...
async def audit(codex, version, output, *, commit=None):
    ...
    class FixtureGateway(OpenRouterGateway):
        ...
        async def upstream(self, request):
            ...
            assert self.fixture_count <= 3, 'Unexpected extra provider request'
            ...
            if active_action == 2:
                assert private_text(1) in assistant, 'Private note was not retained across process restart'
                assert private_text(1, final=True) in assistant, 'Private final was not retained across restart'
            ...
            report['requests'].append({'action': active_action, 'request': self.fixture_count,
                ...
                'previous_private_notes_retained': active_action == 2,
    ...
    try:
        ...
        assert public == [public_text(1), public_text(2)]
        assert [note['text'] for note in notes] == [private_text(1), private_text(1, final=True),
                                                  private_text(2), private_text(2, final=True)]
        assert action_notes == [1, 1, 2, 2]
        assert calls == ['chess_status', 'chess_comment'] * 2
        assert len(report['requests']) == 6
        assert len(private_text(1)) > bridge.MAX_PUBLIC_TEXT
        assert all(gateway.request_count == 3 and gateway.budget_check_count == 3
        ...
        report.update(success=True, checkout_clean=clean and final_clean, exact_commit=exact,
            ...
            long_private_note_exceeds_public_limit=True, completed_actions=2,
