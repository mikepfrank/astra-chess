...
The complete record is `codex-vs-wendy-2026-09-05.pgn`.
...
- `board_scratchpad.py` applied only explicitly supplied square edits. Each of the 36 White moves was previewed as a PNG and inspected before play. The helper has no knowledge of chess legality or attacks.
- A helper agent wrote only `export_game_journal.py`, which copies the manually recorded SAN labels into a PGN file. It supplied no chess advice and did not operate the browser.
